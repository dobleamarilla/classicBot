...
if (BotName=='SecreHit')  Token = process.env['BOT_TOKEN'];  
		...
		default:
			switch (acc){
				...
				case 'Principal':	
					...
					return;
				case "Botiga":
					botsendMessage(msg, 'Opcions de Botigues',{reply_markup: JSON.stringify({keyboard: [
...
function tecnic(msg, estat , TipTep){
	...
	if(msg.data===undefined || msg.data.split(' ')[1].length==0){
		botsendMessage(msg, 'Opcions de Botigues',{reply_markup: JSON.stringify({inline_keyboard: [
...
function incidencies(msg, estat , TipTep){
	...
	if(msg.data.split(' ')[1]=='AvisSi'){
		...
		conexion.recHit(estat[0].Valor, Sql).then(info => {
			botsendMessage(msg, 'Incidencies',{reply_markup: JSON.stringify({inline_keyboard: [
