...
async function enviarKeyboardTurnos(arrayTurnos: Array<any>, idTrabajador: number, idCodigo: string)
{
    var lineas = [];

        for(let i = 0; i < arrayTurnos.length; i++)
        {
            var nuevoId = await conexion.recHit('hit', `INSERT INTO bot_auxiliar (dato1, dato2, dato3, dato4) VALUES ('${nombreTablaPlanificacio(getLunes(arrayTurnos[i].fecha))}', '${arrayTurnos[i].idPlan}', ${idTrabajador}, '${idCodigo}'); SELECT SCOPE_IDENTITY() as id `);
...
bot.on('message', function(msg) {
    if(msg.text.toLowerCase() == 'revisar')
    {
        mainBot(msg, false).then(sesion=>{
            if(!sesion.error)
            {
                let sqlCodisAccio = "select top 1 IdCodigo as idCodigo, Param1 as idTienda, DAY(CONVERT(datetime, Param3, 105)) as dia, MONTH(CONVERT(datetime, Param3, 105)) as mes, YEAR(CONVERT(datetime, Param3, 105)) as year, Param3 as entrada, Param4 as salida, (select nom from Dependentes where codi = Param2) as nombreTrabajador, Param2 as idTrabajador from CodigosDeAccion where tipoCodigo = 'FICHAJE' AND Param9 IS NULL order by TmStmp desc";

                conexion.recHit(sesion.database, sqlCodisAccio).then(async res=>{
                    
                    await conexion.recHit(sesion.database, "UPDATE CodigosDeAccion SET Param9 = 'NO CONTESTAR' WHERE idCodigo = '" + res.recordset[0].idCodigo + "'");
                    if(res.recordset.length > 0)
                    {
                        ...
                        conexion.recHit(sesion.database, sql).then(async resTurnos=>{
                            ...
                            bot.sendMessage(msg.chat.id, `${res.recordset[0].nombreTrabajador}. \nEntrada: ${res.recordset[0].entrada}\nSalida: ${res.recordset[0].salida} `, await enviarKeyboardTurnos(resTurnos.recordset, res.recordset[0].idTrabajador, res.recordset[0].idCodigo));
                        });  
                    }
                    else
                    {
    ...
    if(callbackData.acc == 'updTurno') //UPDATE
    {
        mainBot(msg, false).then(sesion=>{
            if(!sesion.error)
            {
                ...
                conexion.recHit('hit', 'select dato1 as nombreTabla, dato2 as idPlan, dato3 as idTrabajador, dato4 as idCodigo from bot_auxiliar where id = ' + callbackData.id).then(res=>{
                    conexion.recHit(sesion.database, `UPDATE ${res.recordset[0].nombreTabla} SET idEmpleado = ${res.recordset[0].idTrabajador} WHERE idPlan = '${res.recordset[0].idPlan}'
                                                      UPDATE CodigosDeAccion SET Param9 = 'REVISADO' WHERE idCodigo = '${res.recordset[0].idCodigo}';
                                                        `).then(ok=>{
